...
def drill_frame(state: str, phase: int) -> list[list[int]]:
    ...
    rect(m,5,13,4,6,o); rect(m,6,14,3,4,4); rect(m,4,15,2,2,9)
    line(m,10,22,20,22,6); put(m,9,22,6); put(m,21,22,6)
    if state=="thrust":
...
def tile(kind:int) -> list[list[int]]:
    ...
    if kind==0: # solid rock
        rect(m,1,1,14,14,3); rect(m,2,2,5,4,4); rect(m,9,2,4,5,2); rect(m,4,9,7,5,2); put(m,3,8,5); put(m,12,11,4)
    elif kind==1: # brittle rock
        rect(m,1,1,14,14,4); line(m,3,2,7,8,2); line(m,7,8,11,6,2); line(m,7,8,8,13,2); put(m,11,12,5)
    elif kind==2: rect(m,1,1,14,14,3); rect(m,1,1,14,2,6); rect(m,1,13,14,2,2); line(m,3,3,12,3,5)
    elif kind in (3,4,5,6):
        rect(m,1,1,14,14,3); edge={3:(0,0,16,3),4:(0,13,16,3),5:(0,0,3,16),6:(13,0,3,16)}[kind]; rect(m,*edge,6)
    elif kind in (7,8):
        rect(m,1,1,14,14,3); rect(m,0 if kind==7 else 11,0,5,5,6); line(m,4 if kind==7 else 11,4,11 if kind==7 else 4,11,5)
    elif kind==9: rect(m,0,0,16,16,2); rect(m,2,2,12,12,3); rect(m,4,4,7,1,4); put(m,11,9,4)
    elif kind==10:
        rect(m,0,0,16,16,3); rect(m,2,1,3,15,6); rect(m,11,1,3,15,6); line(m,4,4,11,4,5); line(m,4,10,11,10,5)
    elif kind==11:
        rect(m,0,0,16,16,2); line(m,1,8,14,8,8); line(m,4,2,4,8,8); line(m,11,8,11,14,8); put(m,4,2,12); put(m,11,14,12)
    elif kind==12:
        rect(m,0,0,16,16,3); rect(m,4,9,8,6,6); rect(m,6,4,4,5,1); rect(m,7,5,2,4,12); put(m,5,11,8)
    elif kind==13:
        rect(m,0,0,16,16,3); rect(m,2,10,12,4,2); line(m,3,11,12,11,19); put(m,4,10,24); put(m,10,10,24)
    elif kind==14:
        rect(m,0,0,16,16,4); line(m,3,2,7,7,2); line(m,7,7,5,13,2); line(m,9,4,13,10,2); rect(m,2,13,12,2,3)
    else: # interactive highlighted rock: hatch/shine, not hue-only
        rect(m,1,1,14,14,3); rect(m,2,2,5,4,4); line(m,2,12,12,2,12); put(m,3,12,7); put(m,12,3,7)
    return m

def ore_frame(ore:int, phase:int) -> list[list[int]]:
    m=blank(16,16); o=1; rect(m,4,4,8,8,o)
    if ore==0: # ferrite
        rect(m,5,5,6,6,4); put(m,6,5,6); put(m,9,8,2); put(m,5,10,5)
    elif ore==1: # copper thread
        line(m,4,10,7,4,8); line(m,7,4,12,8,8); line(m,12,8,8,12,8); put(m,7,5,12 if phase%2 else 11)
    else: # lumen crystal
        line(m,8,2,13,8,15); line(m,13,8,8,14,15); line(m,8,14,3,8,15); line(m,3,8,8,2,15); put(m,8,6,7 if phase%2 else 12)
    return m

def hazard_frame(kind:int,state:int,phase:int=0)->list[list[int]]:
    m=blank(16,16); o=1
    if kind==0: # steam
        rect(m,4,11,8,4,6); rect(m,6,8,4,3,2); put(m,7,9,12 if state else 5)
        if state==1: line(m,7,7,10,4,12); line(m,10,4,12,2,12)
        if state==2: rect(m,6,1,4,7,7); put(m,5,3,16); put(m,10,5,16)
    elif kind==1: # cable
        rect(m,2,10,12,4,2); line(m,3,11,13,11,19 if state==2 else 8); line(m,6,11,6,5,8); put(m,6,4,24 if state else 5)
        if state==1: put(m,9,8,12); put(m,10,7,12)
        if state==2: put(m,8,6,7); put(m,11,4,7)
    else: # collapse shale
        rect(m,2,5,12,8,4); line(m,3,6,8,10,2); line(m,11,5,8,10,2)
...
def mite_frame(state:int, phase:int)->list[list[int]]:
    ...
    rect(m,5+dx,6,6,5,o); rect(m,6+dx,6,4,4,13); put(m,7+dx,7,16); put(m,9+dx,8,16)
    for x in (4+dx,6+dx,10+dx): line(m,x,10,x-1,13,4)
    if state==2: line(m,5+dx,6,3+dx,3,24); line(m,10+dx,6,12+dx,3,24)
    if state==3: line(m,5+dx,6,2+dx,5,8)
    return m
...
def build_source()->dict[str,Any]:
    ...
    for i,name in enumerate(["carbide_biter","compact_thruster","copper_loop","remote_charge","cargo_frame"]):
        ...
        if i==0: line(m,22,13,30,16,6); line(m,22,19,30,16,6); put(m,29,16,7)
        if i==1: rect(m,3,13,5,6,1); rect(m,4,14,3,4,4); put(m,3,16,12)
        if i==2: line(m,10,11,20,11,8); line(m,10,11,10,20,8); line(m,10,20,20,20,8); line(m,20,11,20,20,8)
        if i==3: rect(m,17,22,5,5,1); rect(m,18,23,3,3,8); put(m,19,22,24)
        if i==4: line(m,9,22,21,22,6); line(m,10,21,10,24,6); line(m,20,21,20,24,6)
...
def make_scene(source:dict[str,Any])->Image.Image:
    ...
    # tunnel negative space, pipes, supports, lamps
    rect_scene(im,0,238,640,122,1); rect_scene(im,0,245,640,5,6)
    ...
    # HUD bands; integer pixels only, deliberately no text/font dependency.
    rect_scene(im,14,14,215,48,1); rect_scene(im,18,18,207,40,3)
    for i,c in enumerate([24,19,9,11]): rect_scene(im,28+i*46,31,34,8,c)
    rect_scene(im,478,14,148,48,1); rect_scene(im,482,18,140,40,3); rect_scene(im,492,30,116,10,12)
    return im

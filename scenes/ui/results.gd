...
func _ready() -> void:
	...
	var discoveries := ", ".join(Array(result.discovery_ids).map(func(id): return String(id))) if not result.discovery_ids.is_empty() else "-"
	$Panel/Details.text = (tr("results.summary") % [outcome, result.duration_ms / 1000.0, "\n".join(cargo_lines), settlement.banked_resources.scrap, settlement.banked_resources.data, settlement.lost_resources.scrap, settlement.lost_resources.data, discoveries, result.damage_taken, result.overheat_count, result.config.seed, result.topology_hash.substr(0, 16), result.route_samples.size()]).replace("\\n", "\n").trim_prefix("\"").trim_suffix("\"")
